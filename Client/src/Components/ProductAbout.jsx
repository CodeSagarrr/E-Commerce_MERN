import React from 'react'

const ProductAbout = () => {
  return (
    <>
      <div>
      <h1 className='my-10 ml-32 text-4xl font-bold '>Shoes </h1>
        <div className='md:flex w-full p-10 border-t-4 mt-10 '>
          
            <div className=' basis-[50%] md:mt-4 mt-2 flex justify-center'>
            <img src="https://png.pngtree.com/png-vector/20230407/ourmid/pngtree-green-transparent-sports-shoes-png-image_6687298.png"  />
            </div>
            
            <div className=' basis-[50%] md:mt-4 mt-2  mr-2 text-[18px] font-medium sm:ml-6 ml-8 '>
                <p className='md:max-w-[94%] max-w-[100%]'>Lorem ipsum dolor sit amet consectetur adipisicing elit. Sed odit commodi recusandae quisquam officia maiores amet suscipit corrupti quibusdam iusto illo, libero architecto? Cum id quaerat iure libero aliquam nulla a temporibus ab accusantium quasi? Incidunt illum,
                <br/> praesentium autem magni iusto molestias nihil odio ut. Culpa sint at quidem pariatur doloribus reiciendis odio, quis dignissimos.
                Lorem, ipsum dolor sit amet consectetur adipisicing elit. Quis cum dolor iure nobis at u 
                <br/>quia. Quisquam, quia. Quisquam, quia. Quisquam, quia. Quisquam, quia. Quisquam,</p>

                <p className='mt-2 md:max-w-[94%]'>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Ipsa quisquam soluta quasi aspernatur sapiente modi fugit, in numquam repellendus nihil saepe,
                <br/> aliquid totam repellat molestias delectus? Tenetur excepturi inventore in mollitia cum unde quisquam illo libero alias numquam. Alias earum provident cupiditate eaque, officia quaerat labore omnis? Eaque placeat molestias fugiat praesentium voluptas, </p>
            </div>
        </div>
        <h1 className='my-6 text-4xl font-bold ml-40'>  Bags </h1>
        <div className='sm:flex w-full  p-6 mt-6  '>
            <div className=' basis-[50%]  sm:ml-6 ml-10 text-[18px] font-medium'>
                <p className=' max-w-[90%] '>Lorem ipsum dolor sit amet consectetur adipisicing elit. Sed odit commodi recusandae quisquam officia maiores amet suscipit corrupti quibusdam iusto illo, libero architecto 
                <br/> praesentium autem magni iusto molestias nihil odio ut. Culpa sint at quidem pariatur doloribus reiciendis odio, quis dignissimos.
                Lorem, ipsum dolor sit amet consectetur adipisicing elit. Quis cum dolor iure nobis at ut
                <br/>quia. Quisquam, quia. Quisquam, quia. Quisquam, quia. Quisquam, quia. Quisquam,</p>

                <p className='mt-2 max-w-[90%] '>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Ipsa quisquam soluta quasi aspernatur sapiente modi fugit, in numquam repellendus nihil saepe,
                <br/> aliquid totam repellat molestias delectus? Tenetur excepturi inventore in mollitia cum unde quisquam 
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolores maiores quibusdam tempore. Nisi amet recusandae pariatur, exercitationem consequatur porro libero?</p>
            </div>
            <div className=' basis-[50%] mr-26  flex justify-center '>
            <img src="https://img.freepik.com/premium-photo/school-preparations-flawless-backpack-white-background_450505-856.jpg?ga=GA1.1.219956737.1712204919&semt=ais_user"  />
            </div>
        </div>
      </div>
    </>
  )
}

export default ProductAbout
